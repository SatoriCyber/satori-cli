use reqwest::{Url, header::{ACCEPT, CONTENT_TYPE, USER_AGENT, CONTENT_LENGTH, HeaderMap, AUTHORIZATION}};

use super::{errors::SatoriError, OauthResponse, DatabaseCredentials, UserProfile};


/// Generate a JWT token from Satori
pub async fn generate_token_oauth(domain: &str, code: String, code_verifier: String, client_id: &str) -> Result<OauthResponse, SatoriError> {
    let address = format!("{domain}/api/oauth/token");
    
    let url = Url::parse_with_params(&address, &[
...
pub async fn get_database_credentials(domain: &str, client_id: &str, jwt: &str, user_id: &str ) -> Result<DatabaseCredentials, SatoriError> {
...
fn get_headers_no_jwt(client_id: &str) -> HeaderMap {
    ...
    headers.insert(USER_AGENT, format!("satori-cli/{}/{client_id}", env!("CARGO_PKG_VERSION")).parse().unwrap());
