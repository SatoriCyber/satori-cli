...
use super::data::{Login, CLIENT_ID, CredentialsFormat};
use super::errors;
...
pub async fn run(params: Login) -> Result<DatabaseCredentials, errors::LoginError> {
    ...
    let print_url = if params.open_browser { // Need to handle a flow where we unable to open url to print the url
    ...
    let user_info = satori_console::get_user_info(&params.domain, CLIENT_ID, &jwt).await.unwrap();
    let database_credentials = satori_console::get_database_credentials(&params.domain, CLIENT_ID, &jwt, &user_info.id).await.unwrap();
    if !params.write_to_file {
        log::info!("{}", credentials_as_string(&database_credentials, params.format));
    }
...
fn build_state() -> String {
    ...
    (0..12)
        .map(|_| rng.gen_range(b'a'..b'z' + 1)).map(char::from)
        .collect::<String>()
}

fn build_oauth_uri(oauth_domain: &str, local_port: u16, state: String, code_challenge: String) -> Url {
    let redirect_url = format!("http://localhost:{local_port}");
    Url::parse_with_params(format!("{oauth_domain}/{OAUTH_URI}").as_str(), &[
...
pub fn credentials_as_string(credentials: &DatabaseCredentials, format: CredentialsFormat) -> String {
    match format {
        CredentialsFormat::Csv => format!("{},{},{}", credentials.username, credentials.password, credentials.expired_at),
