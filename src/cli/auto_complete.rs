...
fn handle_power_shell(auto_complete_script: &str) -> String {
    ...
    for tool in get_tools() {
        ...
        let (start_index_cur_compl, end_index_cur_compl) =
            get_pwsh_tool_completion_indexes(&auto_complete_string, &full_command_line);
        let curr_complete =
...
fn get_pwsh_tool_completion_indexes(
    auto_complete_string: &str,
    full_command_name_with_suffix: &str,
) -> (usize, usize) {
    let start_index = auto_complete_string
        .find(full_command_name_with_suffix)
        .map(|start_index| start_index + full_command_name_with_suffix.len())
        .unwrap();
    let end_index = auto_complete_string[start_index..]
        .find("break")
